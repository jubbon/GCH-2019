...
def is_need_data(region, address, data_type):
    ...
    path = Path(f"/var/local/citylix.dom/data/{region}/houses/{directory}")
...
def get_brief_data(address, browser, link_element, timeout=30):
    ...
    with open_link_in_new_tab(browser, link_element) as tab:
        try:
            ...
            pass
            # tab.save_screenshot(f"/var/local/citylix.dom/screenshots/brief {address}.png")
            # raise
    return data
...
def get_company(address, browser, link_element, timeout=30):
    ...
    with open_link_in_new_tab(browser, link_element) as tab:
        try:
            ...
            tab.save_screenshot(f"/var/local/citylix.dom/screenshots/company {address}.png")
            raise
    return data
...
def get_passport(address, browser, link_element, timeout):
    ...
    with open_link_in_new_tab(browser, link_element, before_switch_hook) as tab:
        try:
            ...
            tab.save_screenshot(f"/var/local/citylix.dom/screenshots/passport {address}.png")
            raise
    return data
...
def houses(region, data_types, filter_list, skip=None):
    ...
    # browser.save_screenshot("/var/local/citylix.dom/screenshots/1.png")
    ...
    try:
        ...
        for option in select_count.find_elements_by_tag_name('option'):
            pass
        print(f"Setting {option.text} houses per page...", flush=True)
        option.click()
    ...
    while True:
        for elem in browser.find_elements_by_class_name("register-card"):
            ...
            if skip and count <= skip:
                print(f"[{count}] Skipping already available house", flush=True)
                continue
            try:
                # Адрес дома
                ...
                if filter_list and not any(map(lambda s: s in address, filter_list)):
                    print(f"[{count}] Skipping address '{address}'...", flush=True)
                    continue
                print(f"[{count}] Processing address '{address}'...", flush=True)
                ...
                if is_need_data(region_uid, address, "brief"):
                    ...
                    print(f"[{count}] Skipped brief", flush=True)
                ...
                #         data.update(company={})
                # else:
                #     print(f"[{count}] Skipped company", flush=True)
                ...
                #         print(f"{err}", flush=True)
                # else:
                #     print(f"[{count}] Skipped passport", flush=True)

                yield data
            except Exception as err:
                print(f"[{count}] Error: {err}", flush=True)
...
def fetcher_from_gis(region, filter_list=None):
    ''' Загружает данные по жилым домам и рассылает их другим компонентам
    '''
    assert isinstance(region, (list, tuple))
    assert len(region) == 2
    region_uid, region_name = region
    ...
    filename = Path("/data/Реестр жилых домов.json")
    try:
    ...
    print(f"Available {len(available_houses)} houses", flush=True)

    data_types = ("brief", )
    # data_types = ("brief", "company", "passport")
    try:
        print(f"[fetcher_from_gis: {region_uid}] Fetching data from dom.gosuslugi.ru for region '{region_name}'...", flush=True)
        ...
        for n, house in enumerate(houses(region, data_types, filter_list, skip=skip), skip):
            ...
            print(f"[fetcher_from_gis: {region_uid}] Sending data from for region {address['fullname']}...", flush=True)
        ...
        print(f"[fetcher_from_gis: {region_uid}] {err}", flush=True)
        raise
    finally:
        pass
        # wb.save(filename)
